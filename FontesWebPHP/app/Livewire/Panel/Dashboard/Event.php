...
class Event extends Component
{
    ...
    {
        $documents = $this->getEvents(false);
    ...
    public function getEvents($paginate = false)
    {
        ...
        $events->orderBy('event_dh', 'DESC')
            ->orderBy('id', 'DESC')
            ->orderBy('event_dh', 'ASC');

        if ($paginate) {
            return $events->paginate(100, pageName: 'events');
        }
    ...
    protected function querySearch($query)
    {
        ...
        // Só os status do domínio dos EVENTOS: status de nota não pertence a
        // esta fonte e não a zera (ver DashboardStatusScope).
        $statusEventos = DashboardStatusScope::paraEventos($this->search['doc_status'] ?? []);

        if ($statusEventos) {
            $query->whereIn('event_status', $statusEventos);
        }

        // Busca rápida do topo: chave da nota ou protocolo do evento.
